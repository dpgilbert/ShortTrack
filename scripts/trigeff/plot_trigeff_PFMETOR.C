...
void plot_trigeff_PFMETOR (const TString& indir = "/nfs-6/userdata/mt2/V00-08-00_json_Cert_271036-273730/") {
  ...
  TFile* f_out = new TFile(Form("trigeff_PFMETOR%s.root",suffix.Data()),"RECREATE");
  ...
  t_ele->Draw("met_pt>>h_met_num_ele23",ele+"ht > 500. && HLT_SingleEl && (HLT_PFMET100_PFMHT100 || HLT_PFHT300_PFMET100)");

  TH2F* h_axis = new TH2F("h_axis",";E_{T}^{miss} [GeV];Eff of paths ORed",40,100,500,20,0,1);
  ...
  c->SaveAs(Form("trigeff_PFMETOR%s.pdf",suffix.Data()));
  c->SaveAs(Form("trigeff_PFMETOR%s.eps",suffix.Data()));
