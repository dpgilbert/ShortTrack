{
  ...
  TFile* f1 = 0;
  if(doSR) f1 = new TFile("yield_histograms_Zinv.root", "READ");
    
  TH1F* hestimate = (TH1F*)f1->Get("yields_MC");
  TH1F* hdata = (TH1F*)f1->Get("yields_GJ");
  ...
  hestimate->GetYaxis()->SetTitleOffset(0.7);
  ...
  legend->AddEntry( hdata, "data-driven (#gamma+jets)", "PL" );
  legend->AddEntry( hestimate, "Z invisible MC", "PL" );
  ...
  for( int iHT=0; iHT < 5; iHT++ ){
    lHT[iHT-1] = new TLine(2+11*iHT, 0.0, 2+11*iHT, yMax );
  ...
  for( int iHT = 0; iHT < nHTRegions; ++iHT){
    ...
    htBox[iHT] = new TPaveText(0.2+0.15*iHT, 0.9-0.06, 0.30+0.15*iHT, 0.85, "brNDC");
...
//  TH2D* h2_axes_ratio = new TH2D("axes_ratio", "", 10, 0, 44, 10, -3.0, 3.0 );
  ...
  h2_axes_ratio->GetYaxis()->SetTitle("Pred/MC");
